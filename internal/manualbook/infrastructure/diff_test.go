...
func TestReadCh8DecodesTableEscapes(t *testing.T) {
	...
	if err := os.WriteFile(filepath.Join(dir, "sections.tsv"), []byte(index), 0o600); err != nil {
		t.Fatal(err)
	}
	...
	if err := os.WriteFile(filepath.Join(dir, "ch8.md"), []byte(md.String()), 0o600); err != nil {
		t.Fatal(err)
	}
	...
	checkWrittenDiff(t, dir, command, note, rows)
}

func checkWrittenDiff(t *testing.T, dir, command, note string, rows []domain.DiffRow) {
