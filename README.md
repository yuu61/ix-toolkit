...
失われる。** 構成や流れを答えるにはページそのものを見る必要がある。
...
元 PDF へのリンクは、PDF ビューアを開ける**人**しか辿れない。`/ix-manual` を動かすのは
エージェントで、`#page=1057` は辿れないが PNG なら `Read` で開ける。出口が塞がっている
ので、ページを画像にして置いておくと図が読めるようになる。

**pdfbook は画像を作らない。置いてあれば張り、無ければ張らない。** 焼くのは外部の
レンダラの仕事にしてある。こうしておくと変換に要る PDF ツールは `pdftotext` 1 つのままで、
レンダラの有無が変換の成否に影響しない。

**焼くには別途レンダラが要る。** poppler-utils の `pdftoppm` か Xpdf の `pdftopng`。
`pdftotext` だけを単体で入れている環境では**入っていない**ので、まず `pdftoppm -v` で
確かめること（この機能の開発環境がまさにそれで、レンダラが無く実行できていない）。
...
逆に、焼いてから変換し直すだけでリンクは付く（変換は `figures/` を消さない）。

ファイル名は `figures/p<ページ番号>.png`（`.jpg` も可）。レンダラが付けるゼロ詰めの
名前（`p-1057.png` / `p-001057.png`）でも、手で置いた `p1057.png` でも拾う。
**桁数はレンダラと総ページ数で変わる。** 引く側はページ番号から名前を組み立てず、
本文に書かれたリンクの綴りをそのまま使うこと。

囲みは消さない。節を探し当てているのは囲みの中の語なので、画像はその**置き換えではなく
不足の補い**として並ぶ。
...
**全ページ焼くのが簡単で、それで困らない。** 変換側は図と表を判別しない（テキストからは
できない）ので、焼いたページの選び方がそのまま絞り込みになる。容量を惜しむなら
`-f` / `-l` でページを区切れる。
...
参考までに、全 1208 ページのうち画像 XObject を持つのは **361 ページ**だった。この資料の
図はすべてラスタの箱＋ベクタの矢印＋実テキストで描かれており、画像を持たないページに図は
見つからなかったので、この 361 ページがほぼ図のあるページにあたる。焼き漏らしても、その
ページが今までどおり PDF リンクだけになるだけで壊れはしない。

解像度は **150dpi で足りるはず**で、300dpi は容量に見合わない（モデルに渡る画像は縮小
される）。ただし **8pt の図中ラベルが 150dpi で読めるかは未検証**である。この環境には
レンダラが無く 1 枚も焼けていないので、最初の 1 枚で確かめてほしい。

> **`pdfimages` は誤った道具である。** 実測すると画像 XObject は 361 ページに 4383 個
> あり、最も多いのは 160×36 や 218×36 の小片だった。図 1 枚が箱ごとにばらばらの網掛け
> ラスタとして入っているためで、抽出すると**ラベルの無い箱の山**が取れる。必要なのは
> 抽出ではなくページ描画（`pdftoppm` / `pdftopng`）。

ページ画像は本文テキストより直接的な複製物なので、`.gitignore` と同じく**手元限り**で扱う。
