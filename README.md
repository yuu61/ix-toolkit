...
- 取得済みの資料は取りに行かない（PDF は実体の有無、Web はキャッシュに残る版で判定）。2 回目以降は
  変換だけなので 20 秒で終わる。
- `-figures` を付けると、PDF の機能説明書のページ画像も焼く（+72 秒・404 MB）。図の向きや構成を
...
- `build -figures` で機能説明書の全ページを PNG に焼き、囲みの直後に `[ページ画像]` を付ける。
  変換と同じ PDFium が描くので、別の道具は要らない。コマンドリファレンスと Web 由来の冊子には
  効かない（囲みとページリンクを出すのが機能説明書の側だけ。Web は図が SVG で残る）。
...
`build` がしていることは、次のサブコマンドを順に流すのと同じ。プロファイルを較正し直す、
ページ画像を一部だけ焼く、といったときはこちらを使う。
